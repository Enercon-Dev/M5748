...
namespace M5748_Monitor
{
    public partial class MainForm : Form
    {
        ...
        public void Send(DataBuffer buffer, String description, int destination)
        {
        ...
        public int ChosenSource()
        {
            switch (sourceComboBox.Text)
            {
                case "Host": return 0x0F;
                case "Charger": return 0x0B;
                case "Master": return 0x0A;
                case "Batt 1": return 0x01;
                case "Batt 2": return 0x02;
                case "Batt 3": return 0x03;
                case "Batt 4": return 0x04;
                case "Batt 5": return 0x05;
                case "Batt 6": return 0x06;
                default: throw new Exception("wrong source selection");
            }
        }
...
#warning todo
        ...
        private async void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (AutoReqStatusRadioButton.Checked)
            {
                ...
                commands.getHpfStatusTel();
            }
            else if (autoReqChargerRadioButton.Checked)
            {
                commands.getChargerStatusTel();
            }
            else if (autoReqMasterRadioButton.Checked)
            {
                commands.getMasterStatusTel();
            }
        ...
        {
            AnalogTelLable label = sender as AnalogTelLable;
            label.Text = db[label.DBFields[1]].GetIntValue() + "." + db[label.DBFields[2]].GetIntValue();
            if(label.DBFields.Length > 3)
            {
                ToolTip myToolTip = new ToolTip();
                myToolTip.SetToolTip(label, db[label.DBFields[3]].GetStringValue());
            }
        }
        ...
        private void error2Button_Click(object sender, EventArgs e)
        {
            ...
            Send(buffer, "Wrong Length", (int)CommAddress.Non);
        }
        ...
        private void chargerStatusButton_Click(object sender, EventArgs e)
        {
            commands.getChargerStatusTel();
        }

        private void chargerSetButton_Click(object sender, EventArgs e)
        {
            sourceComboBox.Text = "Master";
            commands.setChargerOutputs(outOKCheckBox.Checked, isoFaultCheckBox.Checked, faultCheckBox.Checked, spareOutCheckBox.Checked);
        }

        private void masterStatusButton_Click(object sender, EventArgs e)
        {
            commands.getMasterStatusTel();
        }

        private void ConvStatusButton_Click(object sender, EventArgs e)
        {
            commands.convCommand(inrushEnableCheckBox.Checked, convAEnableCheckBox.Checked, convBEnableCheckBox.Checked);
        }

        private void chargerVersionButton_Click(object sender, EventArgs e)
        {
            commands.getVersionTel(CommAddress.Charger);
        }

        private void masterVersionButton_Click(object sender, EventArgs e)
        {
            commands.getVersionTel(CommAddress.Master);
        }
        private void battVersionButton_Click(object sender, EventArgs e)
        {
            commands.getVersionTel(CommAddress.Batt3);
        }
