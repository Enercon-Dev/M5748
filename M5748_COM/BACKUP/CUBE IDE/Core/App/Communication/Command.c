...
//AckCode decodeRequestedPgnTel(DataBuffer* db, struct TelRequest* tel);
...
static const struct CmdDecodeMap cmdDecodeMap[] =
{
  ...
  //{OPCODE_UART_REQ_PGN,decodeRequestedPgnTel}
};
...
//	    return Ack_WrongLength;
//
//	 uint16_t receivedPgn = getShort(db);
//	 uint8_t SourceAddress = getByte(db);
//	 uint8_t requestedPgn[3] = {0};
//	 getArray(db, requestedPgn, 3);
//
//	 if(receivedPgn == 0xEA00) // RQST PGN HEADER
//	 {
//			SAE_J1939_Read_Request(&j1939, SourceAddress, requestedPgn);
//	 }
//	 else
//		 return Ack_WrongParameter;
//
