...
void RoutFrame(DataBuffer *rxData, CommChannel commCh,
		struct TelRequest *telRequest);
void procesIncomingFrame(DataBuffer *rxData, uint8_t addresses,
		struct TelRequest *telRequest);
void quickSend(uint8_t dest);
void processTelemetryRequest(struct TelRequest *telRequest);
void TP_OnFullMessage(uint8_t *data, uint16_t length); // J1939 Transport Protocol
...
extern J1939 j1939;
extern ENUM_J1939_RX_MSG rxMsgType;
CommChannel comm = UserCanBus;
...
void TP_OnFullMessage(uint8_t *data, uint16_t length) // J1939 Transport Protocol
{
	...
	procesIncomingFrame(&db, 0x0f, &intTelRequest);//TODO: change 0x0f to addr
}
...
DataBuffer* intCanbus_receive(uint32_t ch)
{
	...
	if(j1939.ID_and_data_is_updated)
	{
		if(rxMsgType == RX_MSG_RESP_REQ_PROPRIETARY_A )
			{
			routBufferDB.buffer = j1939.from_other_ecu_proprietary.proprietary_A.data;
			routBufferDB.length = j1939.from_other_ecu_proprietary.proprietary_A.total_bytes;
		...
		{
			routBufferDB.buffer = 0;
			routBufferDB.length = 0;
			routBufferDB.offset = 0;
		}
	j1939.ID_and_data_is_updated = 0;
	//procesIncomingFrame(&db, 0x0f, &intTelRequest); // TODO: send to RoutFrame
	 //RoutFrame(&db, UserCanBus, &intTelRequest);
...
void Command_Handler() {
	...
	RoutFrame(intCanbus_receive(UserCanBus), UserCanBus, &intTelRequest);
...
//    intUart_clearTransmitDelay(ChargerBUart); //it probably be the answer for prev command - next command can be transmited
	...
	if (bIsForRoutOut(commCh, source, dest)) {
		...
		fillArray(&routBufferDB, rxData->buffer + rxData->offset,
				rxData->length - rxData->offset);
		routDest = dest;
		routBufferValid = TRUE;
	} else if (bIsForProcess(commCh, source, dest)) {
...
//
//		if (fillTelemetry(&db, telRequest) != Ack_NoError)
//			return;
		...
		// UART → CAN TP
		CAN_SendProprietary_A(db->buffer, db->length);
