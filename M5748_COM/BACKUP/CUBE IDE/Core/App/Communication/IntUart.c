...
#define UART_TX_EN(enable) HAL_GPIO_WritePin(TX_EN_GPIO_Port, TX_EN_Pin, (enable) ? GPIO_PIN_SET : GPIO_PIN_RESET)
#warning "for B10 only"
//#define UART_TX_EN(enable) HAL_GPIO_WritePin(Fault_LED_GPIO_Port, Fault_LED_Pin, (enable) ? GPIO_PIN_SET : GPIO_PIN_RESET)


#define UART_RX_BUFFER_SIZE 128
static uint8_t  uartRxBuffer[UART_RX_BUFFER_SIZE+10]; 
static uint32_t rxIndex = 0;
struct UartStatistics uartStat[COMM_CHANNELS] = {0};
...
void uart_Enable(uint32_t ch)
{
  ...
  //HAL_UART_RegisterCallback(&huart1, HAL_UART_ABORT_COMPLETE_CB_ID, uart_abortComp_Callback);
  //HAL_UART_RegisterCallback(&huart1, HAL_UART_ABORT_RECEIVE_COMPLETE_CB_ID, uart_abortRx_Callback);
