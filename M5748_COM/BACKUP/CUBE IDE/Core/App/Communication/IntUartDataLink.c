...
static uint8_t tx_Buffer[COMM_CHANNELS][TX_BUFFER_SIZE];
static DataBuffer txBuffer[COMM_CHANNELS] = {0};
static uint8_t rx_Buffer[COMM_CHANNELS][RX_BUFFER_SIZE];
static DataBuffer rxBuffer[COMM_CHANNELS] = {0};

static uint32_t rxPreamble[COMM_CHANNELS] = {0};
static int bDataReadyForTx[COMM_CHANNELS] = {FALSE};
static Signal_t sLineFree[COMM_CHANNELS] = {0};
static uint32_t transmitDelay[COMM_CHANNELS] = {0};
static void intUart_ClearReceived();
static struct UartDataLinkStatistics dataLinkStat[COMM_CHANNELS] = {0};
...
void intUart_Handle()
{
  ...
  for (ch=0; ch < COMM_CHANNELS; ch++)
  {
#ifdef DEV_MASTER
    if(uart_bIsTransmitting(ch)) //still transmitting
    {
...
#else
    transmitDelay[ch] = 0; //transmit delay mechanizm isn't in use in Slave MCU
#endif
...
DataBuffer* intUart_getTxBuffer(uint32_t ch)
{
  ...
  {
    //if there were a telemetry waiting to be transmitted it is discarded
    bDataReadyForTx[ch] = FALSE;
