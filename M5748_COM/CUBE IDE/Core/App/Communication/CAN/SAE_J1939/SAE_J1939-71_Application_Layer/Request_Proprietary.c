...
ENUM_J1939_STATUS_CODES SAE_J1939_Response_Request_Proprietary_A(J1939* j1939, uint8_t DA) {
	...
	if (length_of_each_field < 9) {
		...
		uint32_t ID = (0x14EF << 16) | (DA << 8) | j1939->information_this_ECU.this_ECU_address;
		uint8_t data[8];
		...
		j1939->this_ecu_tp_cm.control_byte = DA == 0xFF ? CONTROL_BYTE_TP_CM_BAM : CONTROL_BYTE_TP_CM_RTS; /* If broadcast, then use BAM control byte, TODO:WE NEED TO RESTORE THIS FOR RTS IN THE FUTURE */
 ...
 * PGN: 0x00EF00 (61184)
 */
void SAE_J1939_Read_Response_Request_Proprietary_A(J1939* j1939, uint8_t SA, uint8_t data[], uint8_t dataLength) {
	/* Proprietary A have 1 fixed field in the J1939 struct */
        ...
        uint16_t total_bytes = dataLength;
        if (total_bytes > MAX_PROPRIETARY_A)
          dataLength = 0; //efectively dropping a too large message
        
        j1939->from_other_ecu_proprietary.proprietary_A.total_bytes = total_bytes;
