...
void SAE_J1939_Read_Response_Request_Address_Claimed(J1939 *j1939, uint8_t SA, uint8_t data[]) {
	...
	j1939->from_other_ecu_name.identity_number = ((data[2] & b00011111) << 16) | (data[1] << 8) | data[0];
	j1939->from_other_ecu_name.manufacturer_code = (data[3] << 3) | (data[2] >> 5);
	j1939->from_other_ecu_name.function_instance = data[4] >> 3;
	j1939->from_other_ecu_name.ECU_instance = data[4] & b00000111;
	...
	j1939->from_other_ecu_name.industry_group = (data[7] >> 4) & b0111;
	j1939->from_other_ecu_name.vehicle_system_instance = data[7] & b00001111;
