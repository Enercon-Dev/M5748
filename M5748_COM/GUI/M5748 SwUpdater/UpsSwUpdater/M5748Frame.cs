...
namespace M1787_Monitor
{
    public class M5748Frame : BasicFrame
    {
        ...
        {
            //this function creates frame from data buffer without magic number, length and CRC
            // sutable for getting frame from PGN

            int dataLength = data.Length;
            int frameLength = 4 + dataLength + 1;


            Destination = data[0] & 0x0F; //is it correct source <-> dest
            Sourse = (data[0] >> 4) & 0x0F;

            buffer = new List<byte>(frameLength);

            buffer.AddRange(telemMagicNumber);
            buffer.Add((byte)dataLength);
            buffer.AddRange(data);
            buffer.Add(0); //CRC place holder
