...
/* Public variables ---------------------------------------------------------*/
int bIAP_enabled = FALSE;
iap_status_t IAP_status = IAP_DISABLED;
...
static void IAP_wrq_recv_callback(void *_args, struct udp_pcb *upcb, struct pbuf *pkt_buf, const ip_addr_t *addr, u16_t port)
{
      ...
      (IAP_tftp_extract_block(pkt_buf->payload) == (args->block + 1)))
  {
    ...
    //TODO: this impamentation assumes that the tftp data size is 512 bytes. 
    // receiving bigger packet will brake the program !
    ...
    /* Write received data in Flash but only if the data does not excead the max alowed file size*/
    if ((Flash_Write_Address + count*4) < USER_FLASH_END_DATA_ADDRESS)
    {
      FLASH_If_Write(Flash_Write_Address, data_buffer ,count);
    }
    Flash_Write_Address += (pkt_buf->len - TFTP_DATA_PKT_HDR_LEN);
  ...
  if (pkt_buf->len < TFTP_DATA_PKT_LEN_MAX)
  {
    ...
    pbuf_free(pkt_buf);
    if (Flash_Write_Address >= USER_FLASH_END_DATA_ADDRESS)
    {
      //the recived file is to long - set the length to zero
      data_buffer[0] = 0;
      FLASH_If_Write(USER_FLASH_LENGTH_ADDRESS, data_buffer, 1);
      IAP_status = IAP_RECEIVED_LENGTH_ERROR;
      return;
    }
    //save the data length into flash:
    data_buffer[0] = Flash_Write_Address-USER_FLASH_FIRST_PAGE_ADDRESS;
    FLASH_If_Write(USER_FLASH_LENGTH_ADDRESS, data_buffer, 1);
#ifdef USE_LCD
    ...
    //check the received file CRC
    if (checkFlashCrc(1) < 0)
    {
      data_buffer[0] = 0;
      FLASH_If_Write(USER_FLASH_LENGTH_ADDRESS, data_buffer, 1); //the file is corrupted - set the file lenght to 0 
      IAP_status = IAP_RECEIVED_CRC_ERROR;
    }
    else
    {
      IAP_status = IAP_RECEIVED_OK;
    }
  }
  else
  {
